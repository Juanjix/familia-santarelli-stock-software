...
import {
  ...
  AlertTriangle, CheckCircle2, ChevronRight, Boxes, Package,
} from "lucide-react"
...
export function ManagerDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    return { totalProducts, totalUnits, lowStockAlerts }
  }, [products])
  ...
  const hasAlerts = todayStats && (todayStats.readyRepairs > 0 || todayStats.pendingTransfers > 0)
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-auto p-4 md:p-6 space-y-4 md:space-y-6">
        ...
        {/* KPIs operativos — sin información financiera */}
        <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
          <Card>
            <CardContent className="p-5">
              ...
              {todayStats ? (
                <>
                  <p className="text-2xl font-bold">{todayStats.salesCount}</p>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {todayStats.salesCount === 0
                      ? "Sin ventas por el momento"
                      : `venta${todayStats.salesCount === 1 ? "" : "s"} confirmada${todayStats.salesCount === 1 ? "" : "s"}`}
                  </p>
                </>
              ) : (
        ...
        {/* Movimientos recientes y stock por depósito */}
