-- Migration 032: confirm_sale también inserta en coupons para visibilidad
-- en el módulo de Ticket de Cambio existente.
--
-- Cambios respecto a 030:
--   • Al confirmar: INSERT INTO coupons con el mismo ticket_number como code.
--   • Al anular: marca el coupon como usado (is_used=true) para que no
--     pueda canjearse, ya que la venta fue revertida.
--     (No lo elimina — preserva el historial.)
...
BEGIN
  ...
  END IF;
  -- Si no hay cliente registrado, usamos el nombre del vendedor como referencia
  ...
  -- ── Insertar en exchange_tickets (tabla POS) ─────────────────────────────
  ...
  -- ── Insertar en coupons (módulo Ticket de Cambio existente) ──────────────
  -- Usa el mismo código para que sea buscable en ambos módulos.
  INSERT INTO coupons (
...
-- ── void_sale: también invalida el coupon correspondiente ────────────────────
